...
import { Upload, FileText, CheckCircle2, ShieldAlert, Award, Sparkles, ChevronRight, HelpCircle, Merge, UserPlus, AlertCircle, RefreshCw, MapPin } from 'lucide-react';
...
export default function AdminClient({ tournaments }: AdminClientProps) {
    ...
    <div className="view-enter" data-screen-label="Admin Ingestion">
      ...
      <section className="tile tile-lg hero tile-depth-aqua mb-6">
        ...
        <h1 className="display display-1 font-normal tracking-tight text-white mb-4">
          Championship <span className="hl">ingestion</span>
        </h1>
        <p className="lede on-dark max-w-[540px] text-white/90">
          Upload a raw CSV file of the latest championship results. The engine will parse
          results, auto-sync exact profiles, and let you resolve naming ambiguities in real-time.
        </p>
      </section>

      {/* Main Upload Card */}
      {!uploadResult && !commitResult && (
        <form onSubmit={handleUpload} className="tile p-8 bg-white border-2 border-ink shadow-[4px_5px_0_#0d3a52] max-w-2xl mx-auto">
          <h2 className="display display-3 font-normal text-ink mb-6">Upload results</h2>
          ...
          <div className="mb-6 flex flex-col gap-2 select-none">
            ...
            <select
              ...
              className="appearance-none bg-white border-2 border-ink rounded-xl px-4 h-12 font-semibold text-sm text-ink cursor-pointer focus:outline-none focus:ring-2 focus:ring-aqua"
            >
              {tournaments.map(t => (
          ...
          <div className="flex items-center justify-between gap-4 border-t border-ink/10 pt-6">
            <div className="text-[10px] font-semibold text-ink-3 flex items-center gap-1">
              <HelpCircle size={13} />
              <span>CSV must contain athlete_name, club_id, event, time, and place.</span>
            </div>
      ...
      {commitResult && (
        <section className="tile tile-lg p-10 max-w-2xl mx-auto bg-white/90 border-2 border-ink shadow-[5px_6px_0_#0d3a52] text-center select-none animate-fadeIn">
        ...
        <div className="flex flex-col gap-6 animate-fadeIn">
          ...
          <div className="flex justify-between items-center select-none pt-4 border-t border-ink/10">
            <button
              ...
              className="pill bg-white border-2 border-ink text-ink font-semibold text-xs py-2 px-5 rounded-full hover:bg-aqua-pale disabled:opacity-30 disabled:pointer-events-none cursor-pointer"
            >
              Previous Conflict
            </button>
